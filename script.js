...
form.addEventListener("submit", function (e) {
    textInputs.forEach((elm, i) => {
        if (elm.value === "") {
            errorHandle(e, i, elm.getAttribute("placeholder"), " cannot be empty", elm)
        }
        else if (elm.id === "email" && elm.value !== "" && !(emailRegex.test(elm.value))) {
            errorHandle(e, i, elm.getAttribute("placeholder"), " is not in correct format", elm)
        }
        else if (elm.id === "password" && elm.value !== "" && !(passwordRegex.test(elm.value))) {
            errorHandle(e, i, elm.getAttribute("placeholder"), " is not in correct format", elm)
        }
